...
met = fread('C:/Users/klynoe/Documents/toolik/R_outputs/met/biomet_clean.csv')

#df = fread('C:/Users/klynoe/Documents/toolik/R_outputs/fluxes_merged_2025_08_26.csv')
...
met$TIMESTAMP = as.POSIXct(met$TIMESTAMP, format = "%m/%d/%Y %H:%M", tz = "UTC")
met$TIMESTAMP <- format(met$TIMESTAMP, "%Y-%m-%d %H:%M")
met$TIMESTAMP = as.POSIXct(met$TIMESTAMP, format = "%Y-%m-%d %H:%M", tz = "UTC")

start_time <- as.POSIXct("2025-08-31", tz = "UTC")
end_time   <- as.POSIXct("2025-10-01", tz = "UTC")


met <- met %>%
  filter(TIMESTAMP >= start_time & TIMESTAMP <= end_time)

met = merge(tsdf,met,by = "TIMESTAMP",all = T)
...
```{r, warning=FALSE}
flux_dates <- c("2025-08-31",'2025-10-01')
```
...
```{r}
df$met = (met$LWIN_1_1_1.c + met$SWIN_1_1_1.c) - (met$LWOUT_1_1_1.c + met$SWOUT_1_1_1.c)
df$flux = df$H + df$LE
...
```{r, warning=FALSE}
plot(df$ts,df$SWIN_1_1_1.c)
plot(df$ts,df$SWOUT_1_1_1.c)
plot(df$ts,df$LWIN_1_1_1.c,ylim = c(0,800))
plot(df$ts,df$LWOUT_1_1_1.c,ylim = c(0,800))
```
...
# 
plot(df$ts,co2_flux.qc,ylim = c(-25,25));abline(h=15, col= 'red');abline(h=-10, col='red')
```

Clean based on graphs, create new variable (co2_flux.limits), and see cleaned product

```{r, warning=FALSE}
co2_flux.limits = replace(df$co2_flux,(df$co2_flux > 7 | df$co2_flux < -10) & df$TIMESTAMP >= '2024-08-31' | (df$co2_flux > 10 | df$co2_flux < -14),NA)
...
write.csv(x = df,file = 'C:/Users/klynoe/Documents/toolik/R_outputs/toolik_Flux_Met_Clean_sep_25.csv',row.names = F,quote = F)
```
