...
df = fread('C:/Users/klynoe/Documents/toolik/R_outputs/fluxes_merged_2025_until_09.csv')
...
met$TIMESTAMP <- mdy_hm(met$TIMESTAMP, tz = "UTC")  # month/day/year hour:minute
...
dfs = list(tsdf, df, met)
df = Reduce(function(x, y) merge(x, y, by = "TIMESTAMP", all.x = TRUE), dfs)
...
```{r, warning=FALSE}
flux_dates <- c("2025-04-31",'2025-10-01')
```
...
```{r}
df$met = (df$LWIN_1_1_1.c + df$SWIN_1_1_1.c) - (df$LWOUT_1_1_1.c + df$SWOUT_1_1_1.c)
df$flux = df$H + df$LE
...
```{r, warning=FALSE}
co2_flux.limits = replace(df$co2_flux,(df$co2_flux > 15 | df$co2_flux < -13) & df$TIMESTAMP >= '2025-05-01' | (df$co2_flux > 13 | df$co2_flux < -14),NA)
