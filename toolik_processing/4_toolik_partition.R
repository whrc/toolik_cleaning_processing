...
help("REddyProc-package")
  ...
  #+++ Load data with one header and one unit row from (tab-delimited) text file
    EddyData.F <- fLoadTXTIntoDataframe('./reddy_mtk.txt')
...
#+++ with all variables needed for processing later
  EddyProc.C <- sEddyProc$new('pond', EddyDataWithPosix.F, c('NEE','CH4','H','LE','Ustar','Rg','Tair','Tsoil','RH','VPD'))
  EddyProc.C$sSetLocationInfo(LatDeg =  72.6695, LongDeg =  -77.9981, TimeZoneHour = -5)
...
#+++ Fill gaps in variables with MDS gap filling algorithm (without prior ustar filtering)
 ...
 #EddyProc.C$sPlotFingerprintY('NEE_f',Year.i = 2024)
...
#  EddyProc.C$sPlotHHFluxes('Reco')
  ...
  fWriteDataframeToFile(CombinedData.F, 'mtk-Results_data.txt', 'data')
