---
title: "Prep for Ameriflux - Pond Inlet"
author: "Kyle Arndt"
date: "2024-02-15"
...
knitr::opts_knit$set(root.dir = 'C:/Users/klynoe/Documents/pond_inlet/') #use this to set your directory
```
...
df = fread(input = "C:/Users/klynoe/Documents/pond_inlet/R_outputs/pond_inlet_Flux_Met_Clean_Aug_Nov_24.csv")
#df = fread(input = "C:/Users/karndt.WHRC/Documents/GitHub/pondinlet_cleaning_processing/data/pond_inlet_Flux_Met_Clean_Aug_Nov_24.csv")
...
df$airT     = na.spline(df$airT,maxgap = gap)
...
df$TS_2_38_1_2_1 = na.spline(df$TS_2_38_1_2_1,maxgap = gap)
df$TS_2_38_1_3_1 = na.spline(df$TS_2_38_1_3_1,maxgap = gap)

df$TS_2_38_3_1_1.c  = na.spline(df$TS_2_38_2_1_1.c,maxgap = gap)
df$TS_2_38_3_2_1.c  = na.spline(df$TS_2_38_2_2_1.c,maxgap = gap)
df$TS_2_38_3_3_1.c  = na.spline(df$TS_2_38_2_3_1.c,maxgap = gap)
df$TS_2_38_3_4_1.c  = na.spline(df$TS_2_38_2_4_1.c,maxgap = gap)
df$TS_2_38_3_5_1.c  = na.spline(df$TS_2_38_2_5_1.c,maxgap = gap)
df$TS_2_38_3_6_1.c  = na.spline(df$TS_2_38_2_6_1.c,maxgap = gap)
df$TS_2_38_3_7_1.c  = na.spline(df$TS_2_38_2_7_1.c,maxgap = gap)
df$TS_2_38_3_8_1.c  = na.spline(df$TS_2_38_2_8_1.c,maxgap = gap)
df$TS_2_38_3_9_1.c  = na.spline(df$TS_2_38_2_9_1.c,maxgap = gap)
...
```{r}
summary(df$RH.c) #check on the data coverage after filling small gaps

#save off a copy of the newly merged data
write.csv(x = df,file = "gapfill/met_gf_pond_inlet_2024.csv",row.names = F)
```

Add ERA5 data
```{r}
#load in era5 data for pond inlet for continuous met data (found in Ameriflux folder in bucket)
era = fread('gapfill/ERA5_pond_inlet.csv')
...
ggplot(data = all)+
  ...
  geom_point(aes(date,all$air_temperature-273.15,col='Site'))

airt.mod = lm(formula = all$airT ~ all$airt)
summary(airt.mod)

all$airt.eramod = all$airt*airt.mod$coefficients[2] + airt.mod$coefficients[1]     

ggplot(data = all,aes(airt,air_temperature))+theme_bw()+
  geom_point()+
  geom_smooth(method = 'lm')+
  geom_abline(slope = 1,intercept = 0,col='red',lty=2)
...
ggplot(data = all)+
  geom_point(aes(date,airt,col='ERA5'))+
  geom_point(aes(date,all$air_temperature-273.15,col='Site'))+
  geom_point(aes(date,all$airt.eramod,col='Model'))
```
...
ggplot(data = all)+
  geom_point(aes(date,rad,col='ERA5'))+
  geom_point(aes(date,SWIN_6_10_1_1_1.c,col='Site'))+
  scale_x_datetime(limits = as.POSIXct(c('2024-07-26','2024-12-31')))
...
ggplot(data = all)+theme_bw()+
  geom_point(aes(ts,SWIN_6_10_1_1_1.c,col='site'))+
    geom_point(aes(ts,rad,col='era5'))+
  scale_x_datetime(limits = as.POSIXct(c("2024-08-01","2024-08-10")))
...
ggplot(data = all)+
  geom_point(aes(date,le,col='ERA5'))+
  geom_point(aes(date,LE.c,col='Site'))+
  geom_point(aes(date,le.eramod,col='Model'))
...
ggplot(data = all)+
  geom_point(aes(date,h,col='ERA5'))+
  geom_point(aes(date,H.c,col='Site'))+
    geom_point(aes(date,h.eramod,col='Model'))
```
...
ggplot(data = all)+
  geom_point(aes(date,vpd,col='ERA5'))+
  geom_point(aes(date,all$VPD,col='Site'))

vpd.mod = lm(formula = all$VPD ~ all$vpd)
summary(vpd.mod)

all$vpd.eramod = all$vpd*vpd.mod$coefficients[2] + vpd.mod$coefficients[1]    

ggplot(data = all,aes(vpd,VPD))+theme_bw()+
  geom_point()+
  geom_smooth(method = 'lm')+
  geom_abline(slope = 1,intercept = 0,col='red',lty=2)
```

#reduce down to pond inlet size and add to final data set
```{r}
...
write.csv(x = df,file = './pond_inlet_2024_era.csv',row.names = F,quote = F)
...
Plot comparing pond inlet ERA data to site level data
```{r}
#air T
airt = ggplot(data = all,aes(airt,airT))+theme_bw()+
...
```{r}
png(filename = 'C:/Users/klynoe/Documents/pond_inlet/figs/era5_pond_inlet_comparisons.png',width = 6,height = 4,units = 'in',res = 1500)
