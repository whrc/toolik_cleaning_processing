rm(list = ls())
Sys.setenv(TZ = 'UTC')
setwd('C:/Users/klynoe/Documents/pond_inlet/gapfill')

library(data.table)
library(ggplot2)

#read the ungapfilled data
dat = fread('./pond_inlet_2024_gf.csv')

#read in the gapfilling output
gfh  = fread('./data/mtk-Results_data.txt',header = T,nrows = 0)
gf   = fread('./data/mtk-Results_data.txt',header = F,skip  = 2,na.strings = c('-9999','-10000','-9.9990e+03'))
names(gf) = names(gfh)
...
write.csv(x = dat,file = './data/mtk_gapfilled_clean_2024.csv',quote = F,row.names = F)
